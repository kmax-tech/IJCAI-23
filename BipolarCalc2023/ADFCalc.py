...
class ParseAndPrepare():
    def __init__(self, inp, desiredSemantics,optimizer):
...
class EvaluateAndInterprete(ParseAndPrepare): #this class inherits the prepared formulas from parseandprepare and evaluates the interpretations
    def __init__(self, formulas, desiredSemantics,optimizer):
        ParseAndPrepare.__init__(self, formulas, desiredSemantics,optimizer)
    ...
    def gammaOp_TwoValOptimized(self, interpretation):
        ...
        twoValuedCompletionRequiredFormulas = [] #for these formulas the two-valued completion approach is required
        ...
        for model in self.preparedFormulas:
            ...
            if modelset.issubset(twoValuedNodesSet):
                ...
                twoValuedCompletionRequiredFormulas.append(model)
        ...
        interpretationGenerator = itertools.product(["True", "False"], repeat=len(undefinedNodes)) #for the undefined nodes the two-valued completions are generated
        # take the first interpretation completion from the generator and evaluate gamma for latter comparison
        firstCompletionValues = next(interpretationGenerator)
        firstInterpretation = dict(zip(undefinedNodes, firstCompletionValues))
        gammaBaseInterpretationEvaluated = self.formulaeEvaluation(twoValuedCompletionRequiredFormulas,{**twoValuedInterpretation, **firstInterpretation})
        # cycle through remaining interpretations and look for differences
        for truthValueTuples in interpretationGenerator:

            currentInterpretationUndefinedNodes = dict(zip(undefinedNodes, truthValueTuples))

            # add the two-valued interpretation from nodes which are not undefined, and eval the values
            currentTwoValuedCompletitionEvaluated = self.formulaeEvaluation(twoValuedCompletionRequiredFormulas,{**twoValuedInterpretation, **currentInterpretationUndefinedNodes})  #the current calculated two-valued interpretation
...
class ControlAndPrint(EvaluateAndInterprete):
    def __init__(self, formulas, desiredSemantics, optimizer):
        EvaluateAndInterprete.__init__(self, formulas, desiredSemantics,optimizer)
...
if __name__ == "__main__":
    x = ControlAndPrint(nodes, semantics,optimizer)
